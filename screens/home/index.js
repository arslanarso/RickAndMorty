...
const Home = ({dispatch}) => {
  ...
  const loadCharacters = async () => {
    try {
      ...
      const response = await fetch(
        `https://rickandmortyapi.com/api/character/${batch}`,
      );
  ...
  const _loadMoreCharacters = async () => {
    try {
      if (fetching === false) {
        ...
        const response = await fetch(
          `https://rickandmortyapi.com/api/character/${nextBatch}`,
        );
        const result = await response.json();
        const _characters = getCharactersArray(result);
  ...
  return (
    <View style={{flex: 1, backgroundColor: '#ECEBED', paddingTop: 70}}>
      <FlatList
        data={characters}
        ...
        ListHeaderComponent={() => (
          <SearchBox width={WIDHT} dispatch={dispatch} />
        )}
